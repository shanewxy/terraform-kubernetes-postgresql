locals {
  ...
  labels = {
    "walrus.seal.io/catalog-name"     = "terraform-kubernetes-postgresql"
  ...
  values = [
...

    {
      ...
      image = {
        ...
        tag        = coalesce(try(length(split(".", var.engine_version)) != 2 ? var.engine_version : format("%s.0", var.engine_version), null), "16.0.0")
      }
      auth = {
